...
// 应用数据格式转换为数据库记录
function storedNoteToDbData(note: Omit<StoredNote, 'extractedAt'>): Omit<Note, 'userId' | 'extractedAt'> {
  return {
    id: note.id,
    title: note.title,
...
export class NotesService {
  ...
  saveNote(note: Omit<StoredNote, 'extractedAt'>, userId: number): StoredNote {
    const dbData = storedNoteToDbData(note);
    const savedNote = NotesData.create(dbData, userId);
